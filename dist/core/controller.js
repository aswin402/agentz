...
export class PrimaryController extends EventEmitter {
     ...
     */
    async startTask(request, hasImages = false) {
    ...
    async executeTask(task, intent) {
        if (!this.currentTaskSlug)
            return;
        ...
        if (result.success) {
            ...
            this.specWorkflow.appendImplementationLog(this.currentTaskSlug, `Task "${task.name}" completed by ${agentType}\n\nOutput:\n${result.output}`);
        }
        else {
    ...
    async executeTasksInParallel(tasks, intent) {
    ...
    // ============================================================================
    async executeWithFallback(subagent, task) {
        let attemptIndex = 0;
        let lastError = "";
        ...
        while (attemptIndex < maxAttempts) {
            ...
            try {
                ...
                this.sharedMemory.updateSubagentStatus(subagent.id, subagent.type, "running", `Attempt ${attemptIndex + 1} using ${model.provider}/${model.model}`);
                this.logDecision({
                    action: "spawn",
    ...
    async executeSubagent(subagent, task) {
        ...
        const result = await callLLM(subagent.model.provider, subagent.model.model, messages, timeoutMs);
        if (!result.success) {
            return {
                success: false,
                error: result.error,
    ...
    async attemptAutoFix(task) {
        ...
        if (debuggerModel) {
            const fixSubagent = this.createSubagent({ ...task, name: `auto-fix: ${task.name}` }, "debugger", debuggerModel);
            ...
            this.specWorkflow.appendImplementationLog(this.currentTaskSlug, result.success
                ? `Auto-fix succeeded for "${task.name}"\n\n${result.output}`
                : `Auto-fix failed for "${task.name}": ${result.error}`);
        }
        else {
            this.specWorkflow.appendImplementationLog(this.currentTaskSlug, `Auto-fix skipped for "${task.name}" — no debugger model available`);
        }
    }
    // ============================================================================
    // Task Finalization
    ...
    determineAgentForTask(task, intent) {
