...
export const AGENT_SPECS: Record<AgentType, AgentSpec> = {
  planner: {
    type: "planner",
    name: "Planner",
...
export class AgentFactory {
  ...
  static getSpecsByCapability(capability: AgentCapability): AgentSpec[] {
    return Object.values(AGENT_SPECS).filter((spec) =>
...
export function classifyIntent(
  ...
  if (debugMatch) agents.add("debugger");
  if (codeMatch) agents.add("coder");
  if (refactorMatch) agents.add("refactor");
  if (reviewMatch) agents.add("reviewer");
  if (docsMatch) agents.add("docs");
  if (securityMatch) agents.add("security");
  if (testMatch) agents.add("tester");
  if (researchMatch) agents.add("researcher");
...
export const FREE_PROVIDERS: Record<string, ProviderInfo> = {
  groq: {
    name: "Groq",
