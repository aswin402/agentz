...
program
  ...
  .action(async (request: string, options: { images?: boolean; project?: string }) => {
...
program
  ...
  .option("-p, --project <path>", "Project root directory", process.cwd())
  .action(async (slug?: string, options?: { project?: string }) => {
...
program
  ...
  .action((slug: string, options: { project?: string; spec?: boolean; plan?: boolean; log?: boolean; summary?: boolean }) => {
    const specWorkflow = new SpecWorkflow(options.project);
...
program
  ...
  .action((options: { project?: string }) => {
    ...
    for (const [type, spec] of Object.entries(AGENT_SPECS)) {
      ...
      const chain = configManager.getAgentChain(type as keyof typeof AGENT_SPECS);
...
program
  ...
  .action((options: { project?: string; clear?: boolean }) => {
    const sharedMemory = new SharedMemory(join(options.project || process.cwd(), ".agentz/runtime"));
...
program
  ...
  .option("-p, --project <path>", "Project root directory", process.cwd())
  .action(async (request: string, options: { project?: string }) => {
    ...
    try {
      ...
      const messages = [
        { role: "system" as const, content: systemPrompt },
        {
          role: "user" as const,
          content: [
            { type: "text" as const, text: "Describe the visible facts in this image. Focus on layout, text, UI elements, colors, and content." },
            {
