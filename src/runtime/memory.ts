...
export class SharedMemory {
  ...
  constructor(basePath: string = ".agentz/runtime") {
  ...
  writeEntry(entry: SharedMemoryEntry): void {
    ...
    const existingIndex = existing.findIndex(
      (e) => e.agentId === entry.agentId && e.agentType === entry.agentType
    );

    let updated: SharedMemoryEntry[];
    if (existingIndex >= 0) {
      updated = [...existing];
  ...
  private createDefaultBoulder(): BoulderState {
    return {
      activeTask: "",
      startedAt: new Date(),
      planFile: "",
  ...
  private formatDecision(decision: PrimaryDecision): string {
  ...
  private serializeSharedMemory(entries: SharedMemoryEntry[]): string {
    return entries.map((e) => this.formatEntry(e)).join("\n");
  }

  private parseSharedMemory(content: string): SharedMemoryEntry[] {
  ...
  private parseEntry(section: string): SharedMemoryEntry | null {
    ...
    for (const line of lines.slice(1)) {
      ...
      if (trimmed.startsWith("### ")) {
        ...
        if (currentSection === "work performed") {
          ...
          entry.status = value.replace(/[\[\]]/g, "").toLowerCase() as SubagentStatus;
        }
      }
    }
  ...
  private parseDecisionLog(content: string): PrimaryDecision[] {
    ...
    for (const section of sections) {
      try {
        ...
        const decision: PrimaryDecision = {
          action: action.toLowerCase() as PrimaryDecision["action"],
          timestamp: new Date(timestamp),
          reason: "",
        };
  ...
  private parseLearningsFile(content: string): string[] {
    const learnings: string[] = [];
...
export function createSharedMemory(basePath?: string): SharedMemory {
